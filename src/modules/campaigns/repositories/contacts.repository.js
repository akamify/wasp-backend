...
function findContactsByTags({ workspaceId, wabaId, tags, tagMatch, select, limit }) {
    ...
    return Contact.find({ workspaceId, wabaId, tags: tagQuery, phone: { $type: "string", $ne: "" } })
        .select(select || "_id phone name email company language attributes")
...
function findContactsByAttributeFilters({ workspaceId, wabaId, filters, limit }) {
    const query = { workspaceId, wabaId, phone: { $type: "string", $ne: "" } };
    if (filters?.length) query.$and = filters;
    return Contact.find(query)
        .select("_id phone name email company language attributes tags")
        .sort({ updatedAt: -1, _id: 1 })
        .limit(Math.min(Math.max(Number(limit || 50000), 1), 50000))
        .lean();
}
